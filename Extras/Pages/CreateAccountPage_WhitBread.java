...
public class CreateAccountPage_WhitBread extends BaseClass{
	...
	// below method is for to check whether Email is not validated page is diplayed or not
	...
	public void LoginToSigninTab() throws InterruptedException {
		username.sendKeys(DataProviderFactory.getExcelconfig().getStringData("Login", 0, 0));
		...
		System.out.println("clicked on Connect to wifi button");
	...
	public boolean buttondisplayed1() {
		boolean b=driver.findElement(By.xpath("//nav[@class='navigation']//span[@class='menu-text'][contains(text(),'VIDEOS')]")).isDisplayed();
		/*String expected="VIDEOS";
		String actual=driver.findElement(By.xpath("//nav[@class='navigation']//span[@class='menu-text'][contains(text(),'VIDEOS')]")).getText();
	...
	public boolean buttondisplayed() {
		...
		boolean c=driver.findElement(By.xpath("/html[1]/body[1]/div[1]/div[1]/header[1]/nav[1]/ul[1]/li[8]/a[1]/span[1]/span[1]")).isDisplayed();
		//System.out.println(actual);
		/*sa1.assertEquals(actual, expected, "Page Title is displaying as : " + "actualTitle");
		if(sa1.equals(false)) {
			if (((ITestResult) sa1).getStatus() == ITestResult.FAILURE) {
	...
	//below method for prividing a email address after performing  a forgot password  link
    ...
    //below is the method for to click on the Register Device Again for Email not validated page
