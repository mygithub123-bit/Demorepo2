...
public class CreateAccountPage {
	...
	{
		String Actual=driver.findElement(By.xpath("//div[@class='alert alert-warning']")).getText();
		String Expected="";
		//Assert.assertTrue(driver.getTitle().contains("Welcome back"));
		System.out.println(Actual);
		
		Assert.assertEquals(Actual, Expected, "Page Title is displaying as :");
	...
	String actualTitle=driver.findElement(By.xpath("//h4[contains(text(),'You now have 15 minutes to validate your account')]")).getText();
	...
	Assert.assertEquals(actualTitle, expectedTitle, "Page Title is displaying as :");
		...
		/*if(actualTitle.equalsIgnoreCase(expectedTitle)) {
			...
			Logger.log(LogStatus.PASS,"check page title", "Test Passed" +expectedTitle);
		}
			else
			{
				logger.log(LogStatus.FAIL, "Test failed due to incorrect title","Page title is" +actualTitle);
			}*/
